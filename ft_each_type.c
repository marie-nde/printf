...
/*   Updated: 2020/02/01 13:33:55 by mnaude           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_is_str(char **arg, char **width, t_struct *s_flags)
{
	...
	if (s_flags->width > ft_strlen(*arg))
...
{
	if (s_flags->width > 0 && s_flags->width >= ft_strlen(*arg) &&
	((s_flags->width > s_flags->point && s_flags->zero == ' ') ||
	(s_flags->width > 0 && s_flags->point <= ft_strlen(*arg))))
		*width = ft_fill_str(s_flags->zero, s_flags->width - ft_strlen(*arg));
	else if (s_flags->width > s_flags->point && s_flags->point == 0
	&& s_flags->zero == '0' && s_flags->width >= ft_strlen(*arg))
	{
		*arg = ft_ptrcut(*arg, ft_strlen(*arg) - 2);
		*precision = ft_ptrzero(s_flags->width - ft_strlen(*arg));
		*arg = ft_strjoin(*precision, *arg, s_flags);
	}
	else if (s_flags->point > s_flags->width && s_flags->point >=
	ft_strlen(*arg))
	{
		*arg = ft_ptrcut(*arg, ft_strlen(*arg) - 2);
...
{
	if (s_flags->width >= ft_strlen(*arg) && ((s_flags->width > 0 &&
	s_flags->point == 0) || (s_flags->width > s_flags->point && s_flags->point
	<= ft_strlen(*arg))))
		*width = ft_fill_str(s_flags->zero, s_flags->width - ft_strlen(*arg));
	else if ((s_flags->point > 0 && s_flags->width == 0 && s_flags->point >=
	ft_strlen(*arg)) || (s_flags->point >= s_flags->width && s_flags->point >=
	ft_strlen(*arg)))
	{
